//! Explicit folder structure for **snippets** (groups + subgroups), mirroring
//! `folders.rs` for connections. Lets empty snippet folders exist before any
//! snippet is placed in them — Obsidian/Explorer style.
//!
//! Snippets carry their own `group`/`subgroup` names; the snippets-panel tree is
//! the union of these explicit folders and the folders implied by snippets.
//! Renames/deletes here are mirrored onto snippets by the frontend (via the
//! `snippet_rename_group`/`snippet_rename_subgroup` commands).

use serde::{Deserialize, Serialize};
use tauri::{AppHandle, Runtime};
use tauri_plugin_store::StoreExt;

const STORE_FILE: &str = "snippet_folders.json";
...
#[tauri::command]
pub fn list_snippet_folders<R: Runtime>(app: AppHandle<R>) -> Result<Vec<Folder>, String> {
    load_items(&app)
}

#[tauri::command]
pub fn snippet_folder_create_group<R: Runtime>(
    app: AppHandle<R>,
    name: String,
...
#[tauri::command]
pub fn snippet_folder_create_subgroup<R: Runtime>(
    app: AppHandle<R>,
    group: String,
    name: String,
) -> Result<Vec<Folder>, String> {
...
#[tauri::command]
pub fn snippet_folder_rename_group<R: Runtime>(
    app: AppHandle<R>,
    old_name: String,
    new_name: String,
) -> Result<Vec<Folder>, String> {
...
#[tauri::command]
pub fn snippet_folder_rename_subgroup<R: Runtime>(
    app: AppHandle<R>,
    group: String,
    old_name: String,
    new_name: String,
...
#[tauri::command]
pub fn snippet_folder_delete_group<R: Runtime>(
    app: AppHandle<R>,
    name: String,
) -> Result<Vec<Folder>, String> {
    let n = name.trim();
...
#[tauri::command]
pub fn snippet_folder_delete_subgroup<R: Runtime>(
    app: AppHandle<R>,
    group: String,
    name: String,
) -> Result<Vec<Folder>, String> {
